...
export const toggleLastSign = (numbers) => {
	...
	return numbers.map((num, index) => (Number(num) < 0 || index === 0 ? num : `+${num}`)).join('');
};

export const toggleSignOfOperation = (operation) => (operation.startsWith('-') ? operation.slice(1) : `-${operation}`);
